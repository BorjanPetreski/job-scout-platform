#!/usr/bin/env python3
"""dedup.py — seen-log read/write/match for job-scout-pm v3.

seen.jsonl is the dedup source of truth (append-only, one JSON object per line).
Update semantics: appending a record whose url/key matches an earlier one SUPERSEDES it —
readers take the LAST occurrence. This keeps the file append-only while satisfying the
2.6.0 rule that a later scan resolving an `unverified_blocked` lead updates its reason
instead of creating a duplicate.

Matching order (2.3.0 rule, invariant #4):
  1. exact Job URL match
  2. normalized 3-part key company|role_title|loc_or_domain
Company+title alone is NEVER a match (Andersen lesson: two distinct Delivery Manager reqs,
different client/city — differing locdom means DISTINCT opportunity).

Statuses: dropped | shortlisted | applied | passed | unverified_blocked
`unverified_blocked` is first-class (SysMap lesson): blocked leads are logged immediately,
even without resolution, so they never cost a repeat fetch-and-block cycle.
Never log roles still pending Borjan's own call.
"""
...
def append(rec: dict, path: Path = SEEN_PATH) -> dict:
...
def update(url_or_key: str, fields: dict, path: Path = SEEN_PATH) -> dict | None:
...
def unsynced(path: Path = SEEN_PATH) -> list[dict]:
...
if __name__ == "__main__":
    ...
    print(f"seen.jsonl: {total_urls} unique URLs, {total_keys} unique keys")
