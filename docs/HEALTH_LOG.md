...
| 2026-07-20 | borjan-pm | JustRemote | NEVER_PRODUCED | **Live-diagnosed, re-confirms a pre-existing quirk.** Fetched the profile's category URL directly and headless; also checked the site's own nav for a renamed category (`remote-manager-exec-jobs`, since "project-management" no longer appears in the category nav) and fetched that too — **also 0 postings, just blog/nav links.** Root cause is the catalog's *already-documented* 2026-07-11 quirk: listings lazy-load on scroll; a static/headless snapshot never sees them, and no API/page-data channel exists. The category-rename theory doesn't hold — the renamed page has the identical lazy-load blocker, so it isn't the fix and wasn't applied. **health.py correctly re-flagged a known, still-unresolved issue — not a false signal.** | None — confirmed no fix currently available (needs the site to change engines, or a scroll-triggering render strategy neither `render.py` nor a plain headless snapshot currently does). Existing quirk note in `catalog/platforms.yaml` stands, unchanged. | — |
