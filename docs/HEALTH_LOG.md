...
| 2026-07-21 | borjan-pm | Jobgether | **none — manual audit, not a health.py flag.** Exactly the documented "known blind spot" (HEALTH_MONITORING.md): `raw: 66` read as plausible with no prior higher baseline to collapse from, so `YIELD_COLLAPSE`/`SELECTOR_SUSPECT` structurally couldn't catch it. | Borjan manually opened the 3 category URLs and reported live counts (156/14/4) far above what the scanner logged. Live-diagnosed: the generic single-fetch harvester only ever hits page 1; Jobgether paginates at 50/page via `?page=N`, discoverable from a Next.js RSC payload embedded in every listing page (`"total":[...],"limit":[0,50],"maxPages":[...]`). `emea/project-manager` alone has 156 postings across 4 pages — page 1 alone surfaces ~50, a 3x undercount for that category. | **New dedicated `fetch_jobgether()`** (`core/fetch_boards.py`) replaces the generic harvester for this board: fetches page 1, extracts `maxPages` from the embedded payload (pure helper `jobgether_max_pages()`, unit-tested in `tests/unit_fetch_helpers.py`), then walks `?page=2..maxPages` (capped at `JOBGETHER_MAX_PAGES=20`), falling back to a single-page harvest if the payload shape isn't found. **Verified live end-to-end**: raw 66 -> 169 across the 3 categories; a separate isolated re-check of `project-manager` alone pulled 151/156 (normal live-site drift over ~10 minutes, not a bug). | — |
