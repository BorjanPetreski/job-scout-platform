...
| 2.1 | Schema extensions (`subvariant(+secondary)`, `target_seniority`, `employment_type`, `fte_fraction`, `run.effort(+by_run_type)`, optional floor) + salary.py FTE/floorless wiring | ⬜ | Additive/non-breaking — `borjan-pm` must still validate + scan identically. Plan §3.1 |
