...
> **NEXT: PHASE 2 BUILD (Opus 4.8, steps 2.1→2.11).** The 2.0 pre-build review gate ran 2026-07-16
> (Fable 5, xhigh): 4 blocking findings amended into the plan, D1–D23 untouched (outcome note in
> plan §12.1). The design contract, ordered checklist, and the model/effort handoff live in
> **[PHASE_2_PLAN.md](PHASE_2_PLAN.md)** — see **§12** for per-step effort tiering. Do not re-open
> the settled decisions (D1–D23).
...
| 2.2 | Template format v2: `platform_tiers` + `salary_estimation_heuristics` (text) + `seniority_titles` + lexicon + run-skill enforcement wiring | ⬜ | Plan §3.1/§3.3 |
| 2.3 | Catalog expansion: loader `status` rule first; new boards as full `active: false` + `status: unverified` entries; per-stream slug maps; tiers → templates | ⬜ | Plan §3.2 |
| 2.4 | Template library across the taxonomy: ⭐ tech-core deepest (incl **backend-java**) + a dry-run fixture profile per ⭐ template, business/support/content as groundwork | ⬜ | Plan §2 |
