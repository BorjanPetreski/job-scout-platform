...
| `search.languages` | **DROP** (opt-in `language_mismatch`) ✅ *new* | Generalized from Polish-only → configurable multi-language set. Reads the JD's OWN dominant language. |
| `search.languages` (stated requirement) | FLAG (`stated_language_requirement`) → judgment ✅ *new* | 2026-07-20. Distinct from the row above: an ENGLISH JD that STATES a Polish/German/… proficiency requirement ("Polish C1", "apply in Polish"). `detect_language` reads it as `en` (a target lang) and never fires, so the local-hire barrier reaches the shortlist. Flag-only (scripts flag, Claude decides); a `hard_filters.stated_language_mismatch: [off\|flag\|drop]` toggle is the natural next step if a profile wants mechanical drop (mirror `language_mismatch`). **Dependency:** like every body-reading flag it needs REAL JD text — a `selectolax`-degraded scan (1500-char CSS `jd_text`) starves it silently (the SessionStart hook now guarantees selectolax; see the balance rule). **Residual gap:** a language barrier stated only in the application FORM (not the JD) is unscannable — judgment surfaces it at apply time (Addepto 2026-07-20). |
