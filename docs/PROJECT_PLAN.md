...
| **Self-healing after scan (in-scan remediation)** | health build (3a→3b interstitial) | A bounded Layer-1.5 that auto-recovers mechanically-fixable failures (retry/backoff, direct→headless, endpoint failover) in-run and flags the rest for Layer-2 review — always reporting what it healed (honest-failure preserved). Design in [HEALTH_MONITORING.md](HEALTH_MONITORING.md). |
