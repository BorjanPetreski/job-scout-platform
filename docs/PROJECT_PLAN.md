...
| **Duplicate Passed/Seen rows from dedup twins** (2026-07-20) | engine follow-up | A posting the user reviewed + `User Declined` (shortlist row) can also carry a mechanically-dropped `Filtered Out` twin row (a JD-less dedup twin) in the Passed/Seen Log — TWO Notion rows, one URL, conflicting reasons (found on Vazco + TT MS, borjan-pm 2026-07-20). `reconcile_resolutions_from_passed_seen` then applies both in Notion order and the last wins, so local state can flip to `Filtered Out` even though the user actively declined. Fix candidates: (a) dedup twins should share ONE Passed/Seen row (sync the resolved/authoritative twin, suppress the other), and/or (b) reconcile should rank reasons (user resolution > mechanical) instead of last-wins. Buildable now; parked to keep this PR focused. |
