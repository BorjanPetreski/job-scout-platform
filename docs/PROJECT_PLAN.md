...
| **Board-coverage audit vs. a 20-board reference list** (2026-07-20) | Phase 2 catalog (§3.2) for YC; rejected-evidence log for the other 3 | Borjan asked whether 4 boards were covered: **Y Combinator "Work at a Startup"** — REAL CANDIDATE, add as a `active: false` + `status: unverified` catalog entry per §3.2. Live-checked: anonymous static fetch, no login wall; has role+location URL patterns (`/jobs/role/product-manager/remote`, `/jobs/role/product-manager/europe`, and the `all`/software-engineering equivalents) serving both `borjan-pm` (project-management) and `ani-backend-java` (software-engineering) streams; genuine EU/remote PM signal found live (Cogram/Berlin, Alguna/EMEA) alongside US-skewed listings — same shape as existing Tier 2/3 boards. **Dice** — reject, same bait-and-switch shape as Built In/Remote.co in the existing `rejected_platforms` log (job-scout-pm/config.yaml, invariant #15): a live "Europe" location search returned overwhelmingly US staffing/contract postings (Pittsburgh, Houston, Columbus…) despite the filter. **Key Values** — reject, not a postings aggregator at all (own tagline: "find engineering teams that share your values" — per-company culture pages, no PM/SM/DM category, no listing-churn feed); same shape as Contra's "not a job board" rejection. **Relocate.me** — marginal, likely reject: visible categories (Back End, Lead Developer, QA, DevOps/SRE, Manager, Full Stack, Front End, Team Lead, Support) have no clean PM/delivery/scrum-master bucket, JS-rendered (needs headless), and the actually-curated feed ("The Global Move") is paywalled at $15/mo — same shape as FlexJobs; the visa-sponsorship angle could be worth a second look for `ani-backend-java` specifically before formally rejecting. None of the three rejections are yet copied into a catalog-native rejected list (the only one that exists today is the legacy `job-scout-pm/config.yaml`) — worth deciding in Phase 2 whether `catalog/platforms.yaml` should grow its own `rejected_platforms` section as the source of truth moves off the legacy file. |
