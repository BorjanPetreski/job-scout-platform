...
| **Remote Rocketship: `/publicjobs/` prefix regression fix + query-search permutation harvest** (2026-07-21) | engine follow-up — platform audit, boards 5/6 | Two items Borjan asked to scope-and-log rather than build immediately, so they aren't lost before the next session. **(1) Regression (small, ready to fix next session)**: the same-day `/publicjobs/company/{co}/jobs/{slug}` prefix fix now looks wrong — three fresh live fetches this session (worldwide + two of Borjan's own query URLs) all show un-prefixed `/company/{co}/jobs/{slug}` in the raw server HTML, while `HARVEST_SPECS` requires the prefix. Likely a pre-/post-hydration split (Borjan's real browser shows the prefixed URL after JS runs; the scanner's `direct` fetch never executes JS). Net effect: Remote Rocketship is probably harvesting 0 real postings right now. Next session: fix the regex to accept the un-prefixed form (or both), verify live end-to-end. **(2) Bigger idea (needs real sizing before building)**: Borjan found the site also serves `https://www.remoterocketship.com/?page=1&sort=DateAdded&jobTitle={title}&locations={location}` — a query-search surface supporting free-text role variants and per-country locations (down to individual EMEA countries, not just the "Europe"/worldwide granularity the catalog uses today), where the sign-up wall gates deep pagination per query but not the query itself. His proposal: sweep job-title variants × EMEA country variants (each a small, newest-first result set) to out-harvest the current single-category-URL approach. Not scoped for request-volume/rate-limit/politeness cost yet, and it changes the fetch architecture (new catalog fields, possibly a new `fetch_mode`), not a one-line fix — deliberately left for a dedicated sizing pass, not built reactively. Full live-verification detail in [HEALTH_LOG.md](../docs/HEALTH_LOG.md) Review log (2026-07-21, "Remote Rocketship (5/6) — Borjan's manual re-check"). |
